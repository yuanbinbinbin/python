...
def isExist(anchor_id):
	...
	return False
def updateData(anchor_id,nick_name,game,brief,sex,fans_count,video_count,registerTime,country,province,city):
	try:
		#print "update: "+str(anchor_id)
		conn = mdb.connect(host="localhost",user="root",passwd="123456",db="db_data_platform",port=3306)
		cur = conn.cursor()
		Sqq = "update tb_user_qie set nick_name ='"+str(nick_name)+"', game = '"+str(game)+"', brief = '"+str(brief)+"', sex = '"+str(sex)+"', fans_count = "+str(fans_count)+", video_count = "+str(video_count)+", country = '"+str(country)+"', province = '"+str(province)+"', city = '"+str(city)+"' where anchor_id = '" +str(anchor_id)+"'"
		cur.execute(Sqq)
...
def insertData(anchor_id,nick_name,game,brief,sex,fans_count,video_count,registerTime,country,province,city):
	# print room_id
	# print(nickname)
	# print fans_count
	# print(video_count)
	...
	try:
		...
		conn = mdb.connect(host="localhost",user="root",passwd="123456",db="db_data_platform",port=3306)
		cur = conn.cursor()
		cur.execute("insert into tb_user_qie values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(anchor_id,nick_name,game,brief,sex,fans_count,video_count,registerTime,country,province,city))
		conn.commit()
		cur.close()
		conn.close()
...
while True:
	...
	for items in arr:
		...
		if info1["sex"] == 1:
			sex = "男"
		if isExist(items):
			updateData(info2["anchor_id"],info1["nick_name"],info2["appname"],info1["brief"],sex,info1["fans_count"],info1["video_count"],time.strftime('%Y-%m-%d %H:%M:%S', time.localtime((info1["register_ts"]))),info1["country"],info1["province"],info1["city"])
		else:
			insertCount = insertCount + 1
			insertData(info2["anchor_id"],info1["nick_name"],info2["appname"],info1["brief"],sex,info1["fans_count"],info1["video_count"],time.strftime('%Y-%m-%d %H:%M:%S', time.localtime((info1["register_ts"]))),info1["country"],info1["province"],info1["city"])
	print(insertCount)
	time.sleep(300)	
#2346742
