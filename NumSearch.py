	...
	try:
		...
		for item in result:
			...
			if "000" in str(item) :
				...
				addNum(str(item))
			elif "222" in str(item):
				addNum(str(item))
			elif "333" in str(item):
				addNum(str(item))
			elif "444" in str(item):
				addNum(str(item))
			elif "555" in str(item):
				addNum(str(item))
			elif "666" in str(item):
				addNum(str(item))
			elif "777" in str(item):
				addNum(str(item))
				...
				addNum(str(item))
			if item != 0 :
				if(str(getNum).find(str(item))) < 0:
					getNum = str(getNum) + str(item) + ","
		
		if(str(getNum).count(",") > getNumCount):
			getNumCount = str(getNum).count(",")
	except Exception :
		temp = 1	
